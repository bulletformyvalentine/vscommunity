...
-	This script does the following -   	 	- installs chocolatey 
...
$ScriptLog = Join-Path -Path $ChocolateyPackageInstallerFolder -ChildPath 
"ChocolateyPackageInstaller.log" 
$ChocolateyInstallLog = Join-Path -Path $ChocolateyPackageInstallerFolder -ChildPath 
...
{ 
    ...
    { 
        New-Item -Path $ChocolateyPackageInstallerFolder -ItemType directory | Out-
Null 
    } 
} 
...
#  
# Description: 
#  - Writes specified string to the console as well as to the script log (indicated by 
$ScriptLog). 
# 
# Parameters: 
...
{ 
    ...
    $timestampedMessage = "[$([System.DateTime]::Now)] $Message" | % {         if (-not $LogFileOnly) 
        { 
            Write-Host -Object $_ 
        } 
...
# 
# Return: 
#  - If installation is successful, then nothing is returned. #  - Else a detailed terminating error is thrown. # 
# Notes: 
...
net.webclient).DownloadString('https://chocolatey.org/install.ps1')) | Out-Null  
    WriteLog 'Success.' 
} 
...
{ 
    ...
    { 
        WriteLog 'No packages were specified. Exiting.'         return         
    }  
    ...
    { 
        ...
        # Install git via chocolatey.         choco install $package --force --yes --acceptlicense --verbose --allow-emptychecksums | Out-Null           if (-not $?) 
        { 
            $errMsg = 'Installation failed. Please see the chocolatey logs in 
%ALLUSERSPROFILE%\chocolatey\logs folder for details.'             throw $errMsg  
        } 
...
{ 
    $errMsg = $Error[0].Exception.Message     if ($errMsg) 
    { 
    ...
    # throw so that startChocolatey.ps1 can handle the state correctly.     throw }  
